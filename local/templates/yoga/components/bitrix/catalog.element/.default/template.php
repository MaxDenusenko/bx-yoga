...
<div class="ticket">
    ...
    <div class="ticket__wrapper default-container">
        ...
        <div class="ticket__content">
            ...
            <hr class="wp-block-separator m-5">
        </div>

        <div class="services-block__price" style="justify-content: center; margin: 10px 0 30px;">

        <?php foreach ($arResult['OFFERS'] as $k => $OFFER) : ?>

            <div class="services-block__price-block <?= ((count($arResult['OFFERS']))-1) == $k ? 'services-block__price-block_last' : '' ?>">
                <p><?=$OFFER['NAME']?></p>
                <span><?=$OFFER['PREVIEW_TEXT']?></span>
                <button data-toggle="modal" data-target="#offer_pay_<?=$arResult['ID'].'-'.$OFFER['ID']?>" class="btn-general">
                    Оплатить <?= CurrencyFormat($OFFER['PRICES']['RUB']['VALUE'], "RUB") ?>
                </button>
            </div>

            <div class="modal fade" id="offer_pay_<?=$arResult['ID'].'-'.$OFFER['ID']?>" tabindex="-1" role="dialog" aria-hidden="true">
